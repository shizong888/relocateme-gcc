...
export default function UAEGoldenVisaPage() {
  const data = immigrationData["uae-golden-visa"];
  ...
  return (
    <>
      ...
      <section className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-white">
        <Image
          src={data.heroImage}
          alt={`${data.name} immigration services`}
          fill
        ...
        <div className="container mx-auto px-4 md:px-6 pt-20 pb-8">
          ...
          >
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-light leading-tight text-left">
              Your <span className="text-[hsl(var(--brand))]">UAE Golden Visa Journey</span>
            </h2>
            <p className="text-lg text-gray-600 mt-4">
              Achieve peace of mind and long-term residency with RelocateMe's structured, compliance-first process.
            </p>
          </motion.div>
        </div>
      ...
      <WhyChooseSection
        ...
        items={data.whyChoose.items.map((item, index) => {
          const icons = [<Building2 key={0} size={32} />, <Users key={1} size={32} />, <TrendingUp key={2} size={32} />];
          return {
            icon: icons[index],
            title: item.title,
      ...
      <ContactForm
        ...
        imageSrc="https://images.unsplash.com/photo-1512453979798-5ea266f8880c?w=2070&q=80"
        imageAlt={`${data.name} services`}
      />
